...
<body>
			...
			<table id="listTable">
				...
				<tbody>
					...
					<tr>
						...
						<td class="subject">
							<a href="<c:url value="boardDetail/${article.boardDetailNo}"/>">
							${article.subject} </a> 
						</td>
						<td>
							${article.name}
					...
					<tr>
						<td colspan="4" style="text-align:center;border:0px;">
							<a href="${pageContext.request.contextPath}/work/boardWrite" style="float:right;font-size:15px;"><button
	...
	<c:if test="${startPage > pageBlock}">
		<a href="<c:url value="/work/boardMain?pageNum=${startPage - pageBlock}"/>">이전</a>
	</c:if>
	
	<c:forEach var="i" begin="${startPage}" end="${endPage}">
		<a href="<c:url value="/work/boardMain?pageNum=${i}"/>">[${i}]</a>
	</c:forEach>
	
	<c:if test="${endPage < pageCount}">
		<a href="<c:url value="/work/boardMain?pageNum=${startPage + pageBlock}"/>">다음</a>
	</c:if>
</c:if>
		</div>
	</section>
